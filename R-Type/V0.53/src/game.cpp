...
game::game() : sprite_manager(std::make_shared<handlerSprite> ()), title(std::make_shared<handlerSprite> ())
{
}
...
void  game::applyPlayerTurn(int count, bool endMap)
{
  ...
  if (count == 2000)
    {
      this->objects.push_back(std::move(this->m_physique.fireOrder(this->ships[0], DIRECTION::RIGHT)));
      //count = 0;
    }
}
      ...
      while (i != this->ships.size())
        {
          ...
          if (count == 2000 && this->ships[i]->getBoss() == false)
            {
              this->objects.push_back(std::move(this->m_physique.fireOrder(this->ships[i], DIRECTION::LEFT)));
              //count = 0;
            }
