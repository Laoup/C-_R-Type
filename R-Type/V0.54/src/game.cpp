...
game::game() : sprite_manager(std::make_shared<handlerSprite> ())
{
}
...
void  game::applyPlayerTurn(int count, bool endMap)
{
  ...
  if (count == 2000)
    {
      //this->objects.push_back(std::move(this->m_physique.fireOrder(this->ships[0], DIRECTION::RIGHT, false)));
      ...
      while (i != this->ships.size())
        {
          ...
          if (count == 2000)
            {
              //this->objects.push_back(std::move(this->m_physique.fireOrder(this->ships[i], DIRECTION::LEFT, this->ships[i]->getBoss())));
...
int  game::menu(sf::Vector2f arr, int xSizeWindow, int ySizeWindow)
{
  ...
  this->drawable_menu.setSpritePosition((xSizeWindow / 2) - 350, 0);

  this->drawable_menu.addText("Start", 60);
  this->drawable_menu.setTextPosition("Start", (xSizeWindow / 2) - 200, ySizeWindow / 2);
  this->drawable_menu.setColorText("Start", sf::Color::Red);

  bounding_box = this->drawable_menu.getBoundsText("Start");

  if (bounding_box.contains(arr))
    return (1);
  else
    return (0);
}
...
int   game::endScreen(sf::Vector2f  arr, int xSizeWindow, int ySizeWindow)
{
  ...
  this->drawable_menu.setTextPosition("Do You want to Play Again", (xSizeWindow / 2) - 200, ySizeWindow / 2);
  this->drawable_menu.setColorText("Do You want to Play Again", sf::Color::Red);

  this->drawable_menu.addText("YES", 50);
  this->drawable_menu.setTextPosition("YES", (xSizeWindow / 2) - 100, ySizeWindow / 2 + 75);
  this->drawable_menu.setColorText("YES", sf::Color::Red);

  this->drawable_menu.addText("NO", 50);
  this->drawable_menu.setTextPosition("NO", (xSizeWindow / 2) + 100, ySizeWindow / 2 + 75);
...
void  game::initializeTheGame(sf::View view, int xSizeWindow, int ySizeWindow)
{
  ...
  //this->sprite_manager = std::make_shared<handlerSprite>;
  //this->sprite_manager = std::shared_ptr<handlerSprite>();
  this->handler_view.setWindowSize(xSizeWindow, ySizeWindow);
  this->handler_view.setView(view);
