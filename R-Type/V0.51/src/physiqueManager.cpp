  ...
  while (i != vec.size())
    {
      ...
      if (direct == DIRECTION::LEFT)
        vec[i]->move(-1 * (SPEED * 3), 0);
      else if (direct == DIRECTION::RIGHT)
        vec[i]->move(SPEED * 3, 0);
      i++;
    }
}

void  physiqueManager::checkCollision(element *player, element *ennemy, std::vector<object *> &objs, handlerSprite *sprite_manager)
{
...
void            physiqueManager::generateEnnemys(int posSide, std::array<int, 3> &arr, std::vector<ship *> &ships, int xSizeBackground, int xSizeWindow, handlerSprite* sprite_manager)
{
  int           count;
  float         stock;
...
void            physiqueManager::createWaveEnnemys(int posSide, std::array<int, 3> &arr, std::vector<ship *> &ships, handlerSprite* sprite_manager)
{
  std::vector<ship *>::iterator it;
  int                           random_nb;
  int                           tmp;
