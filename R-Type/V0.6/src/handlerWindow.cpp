...
/*void handlerWindow::windowDraw(element *element)
{
  this->window.draw(*element);
}*/

void handlerWindow::windowDraw(drawableClass &drawable)
{
  this->window.draw(drawable);
}

void  handlerWindow::windowDrawMenu(drawableClass &drawable)
{
  std::map<std::string, sf::Text>           map_text;
  std::map<std::string, sf::Text>::iterator it;

  this->window.draw(drawable.getSprite());
  map_text = drawable.getMapText();
  for (it = map_text.begin(); it != map_text.end(); it++)
